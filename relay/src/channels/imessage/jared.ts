...
interface JaredConfig {
  pollInterval?: number; // How often to check for new messages (ms)
  chatDbPath?: string;   // Path to chat.db (default: ~/Library/Messages/chat.db)
}
...
export class JaredAdapter implements ChannelAdapter {
  readonly type = ChannelType.IMESSAGE;

  private config: JaredConfig;
   ...
   */
  private async queryDb(query: string): Promise<string> {
    const { stdout } = await execAsync(
      `sqlite3 "${this.chatDbPath}" "${query.replace(/"/g, '\\"')}"`,
      { timeout: 5000 }
    );
    return stdout;
  }
   ...
   */
  private parseQueryResult(result: string): ChatMessage[] {
