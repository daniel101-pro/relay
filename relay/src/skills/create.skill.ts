...
export class CreateSkill extends BaseSkill {
  ...
  readonly handledIntents = [IntentType.CREATE];
...
Respond with JSON only:
{
  ...
  "estimatedGas": "~X gas"
    ...
    try {
      const response = await this.aiClient.getCompletion(
        'You are a Solidity smart contract expert. Generate secure, gas-efficient contracts following best practices.',
        prompt,
      ...
      if (jsonMatch) {
        return JSON.parse(jsonMatch[0]) as ContractGenerationResult;
      }
   ...
   */
  private generateFromTemplate(contractType: string, requirements: string[]): SkillResult {
    const templates: Record<string, () => SkillResult> = {
   ...
   */
  private inferContractType(message: string): string {
  ...
  private getMultisigResult(requirements: string[]): SkillResult {
  ...
  private getTokenResult(_requirements: string[]): SkillResult {
    return this.success(
      {
        type: 'contract_creation',
        contractType: 'token',
  ...
  private getNFTResult(_requirements: string[]): SkillResult {
    return this.success(
      {
        type: 'contract_creation',
        contractType: 'nft',
  ...
  private getEscrowResult(_requirements: string[]): SkillResult {
    return this.success(
      {
        type: 'contract_creation',
        contractType: 'escrow',
  ...
  private getVaultResult(_requirements: string[]): SkillResult {
    return this.success(
      {
        type: 'contract_creation',
        contractType: 'vault',
  ...
  private getStakingResult(_requirements: string[]): SkillResult {
    return this.success(
      {
        type: 'contract_creation',
  ...
  private getMultisigTemplate(required: number = 2, total: number = 3): string {
...
contract MultiSigWallet {
  ...
  private getTokenTemplate(): string {
    return `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
contract Escrow {
  ...
  private getVaultTemplate(): string {
    return `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
