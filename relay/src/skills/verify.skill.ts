...
export class VerifySkill extends BaseSkill {
  ...
  readonly handledIntents = [IntentType.VERIFY];

  private explorer = getExplorerAPI();
  private provider = getBlockchainProvider();
   ...
   */
  private async verifyTransaction(txHash: string, chainId: number): Promise<SkillResult> {
  ...
  private checkAccountAge(
    recentTxs: Array<{ timestamp: number }>,
    txCount: number
  ): VerificationCheck {
    if (txCount === 0) {
      return {
    ...
    if (recentTxs.length > 0) {
      ...
      const oldestTx = recentTxs.reduce((oldest, tx) =>
  ...
  private checkActivityPatterns(
    recentTxs: Array<{ from: string; to: string | null; value: string; timestamp: number }>,
    address: string
  ): VerificationCheck {
    if (recentTxs.length === 0) {
      return {
   ...
   * Check balance
   */
  private checkBalance(balanceWei: string): VerificationCheck {
   ...
   */
  private checkContractVerification(addressInfo: {
    isContract: boolean;
   ...
   */
  private checkContractSecurity(contractInfo: {
    verified: boolean;
    sourceCode?: string;
    name?: string;
  }): VerificationCheck {
   ...
   */
  private analyzeFunctionCall(input: string): VerificationCheck {
   ...
   */
  private calculateOverallRisk(checks: VerificationCheck[]): RiskLevel {
    ...
    if (highSeverityFails >= 2) return RiskLevel.CRITICAL;
    if (highSeverityFails >= 1) return RiskLevel.HIGH;
    if (mediumSeverityFails >= 2) return RiskLevel.MEDIUM;
   ...
   */
  private generateVerdict(risk: RiskLevel, isContract: boolean): { text: string; emoji: string } {
  ...
  private generateSummary(
    ...
    addressInfo: { isContract: boolean; contractName?: string; transactionCount: number },
    verdict: { text: string }
  ): string {
