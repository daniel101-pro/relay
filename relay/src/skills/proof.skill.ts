...
export class ProofSkill extends BaseSkill {
  ...
  readonly handledIntents = [IntentType.PROOF];
  ...
  private generateReceipt(tx: {
    ...
    gasFee: string;
    network: string;
  }): { text: string; formatted: string; shareable: string } {
...
Generated by Relay
    ...
    const formatted = {
      ...
      status: `✅ ${tx.status.toUpperCase()}`,
      amount: `💰 ${tx.value} (${tx.valueUSD})`,
      from: `📤 From: ${tx.from.slice(0, 8)}...${tx.from.slice(-6)}`,
      to: `📥 To: ${tx.to.slice(0, 8)}...${tx.to.slice(-6)}`,
      date: `📅 ${date}`,
      fee: `⛽ Fee: ${tx.gasFee}`,
