...
export class SafetySkill extends BaseSkill {
  ...
  readonly handledIntents = [IntentType.SAFETY];
  ...
  private analyzeSecurityConcern(
    message: string,
    entities: { addresses: string[]; transactionHashes: string[] }
  ): {
    type: string;
    indicators: string[];
  ...
  private calculateOverallRisk(
    analysis: { severity: string },
    scamCheck: { isLikelyScam: boolean; confidence: number } | null,
    addressCheck: { flagged: boolean } | null
  ): {
    level: RiskLevel;
    score: number;
    confidence: number;
  ...
  private getRecommendations(
    ...
    scamCheck: { isLikelyScam: boolean } | null
  ): string[] {
  ...
  private getEducationalInfo(concernType: string): {
    title: string;
    content: string;
  } {
    const educational: Record<string, { title: string; content: string }> = {
      credential_theft_attempt: {
        title: 'Protecting Your Wallet',
        content:
          'Your seed phrase and private keys should NEVER be shared with anyone. No legitimate service will ever ask for them. These give complete control of your funds.',
      },
      potential_phishing: {
        title: 'Recognizing Phishing',
        content:
          'Phishing attacks create urgency and impersonate trusted sources. Always verify by going directly to official websites, never through links in messages.',
      },
      potential_scam: {
        title: 'Common Crypto Scams',
        content:
          'Doubling scams, fake airdrops, and "guaranteed returns" are common. In crypto, if something promises unrealistic returns, it\'s almost certainly a scam.',
      },
      default: {
        title: 'Stay Safe in Crypto',
        content:
          'Always verify transactions before signing. Use hardware wallets for large amounts. Never rush into decisions under pressure.',
      },
    };
