...
export class ExplainSkill extends BaseSkill {
  ...
  readonly handledIntents = [IntentType.EXPLAIN];
   ...
   */
  private async explainTransaction(txHash: string, chainId: number): Promise<SkillResult> {
   ...
   */
  private formatTransactionResult(txData: {
    hash: string;
    from: string;
    to: string | null;
    value: string;
   ...
   */
  private determineTransactionType(txData: {
    to: string | null;
    value: string;
    input?: string;
    functionName?: string;
    ...
    if (
      txData.input?.startsWith('0x38ed1739') || // swapExactTokensForTokens
    ...
    // NFT transfer
    if (
      txData.input?.startsWith('0x23b872dd') || // transferFrom
  ...
  private generateTransactionExplanation(
    txData: { from: string; to: string | null; status: string },
    txType: string,
    valueEth: string,
    network: string
  ): string {
  ...
  private assessTransactionRisk(
    txData: { status: string; to: string | null },
    valueEth: number
    ...
    if (riskScore >= 2) return { level: RiskLevel.MEDIUM, warnings };
   ...
   * Explain an address
   */
  private async explainAddress(address: string, chainId: number): Promise<SkillResult> {
    // Fetch address info
   ...
   */
  private assessActivityLevel(txCount: number): string {
    if (txCount === 0) return 'no';
  ...
  private assessAddressRisk(
    addressInfo: { isContract: boolean; contractVerified?: boolean; transactionCount: number },
    _recentTxs: Array<{ hash: string }>
  ): { level: RiskLevel; warnings: string[] } {
    const warnings: string[] = [];
    let riskScore = 0;
   ...
   * Explain an ENS name
   */
  private async explainENS(ensName: string): Promise<SkillResult> {
    try {
