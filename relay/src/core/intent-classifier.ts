...
export class IntentClassifier {
  ...
  async classify(message: string): Promise<ClassifiedIntent> {
    ...
    try {
      const result = await this.aiClient.getStructuredCompletion(
  ...
  private quickClassify(message: string): ClassifiedIntent | null {
    ...
    // Confirmation messages -> route to send skill to handle confirmation
    const confirmPatterns = ['confirm', 'yes', 'yep', 'yeah', 'yea', 'do it', 'send it', 'go ahead', 'proceed', 'ok', 'okay', 'sure', 'bet', 'lets go', "let's go", 'aight'];
    if (confirmPatterns.some(p => msg === p || msg === p + '!' || msg === p + '.')) {
      return {
        type: IntentType.SEND,
        confidence: 0.9,
        entities: this.extractEntities(message),
        reasoning: 'Confirmation detected - likely confirming a pending transaction',
      };
    }
   ...
   */
  private createUnknownIntent(reasoning: string): ClassifiedIntent {
    return {
   ...
   */
  private extractEntities(message: string): ClassifiedIntent['entities'] {
